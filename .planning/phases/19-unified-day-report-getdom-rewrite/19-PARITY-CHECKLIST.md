...
## Preserved Behaviors (35 + 5 added by the 19-REVIEW fix pass)
...
| 22 | T-16-20/T-16-22 | `truncateHazardLabel`/`HAZARDS_LABEL_MAX_CHARS=60`, truncated **before** escaping (bounds source chars, not entity expansions) | MMM-SPCOutlook.js:623, MMM-SPCOutlook.js:627-632 | MMM-SPCOutlook.js:269-275 — relocated verbatim above every render branch (WR-06), same body; applied at the one shared compact-segment call site, MMM-SPCOutlook.js:746 (line references corrected during 19-08 sign-off). **CORRECTED (19-REVIEW WR-04):** the compact-segment call site was not the only one — `detailColoredSpan` applied the same bound to content that ALREADY carried the 13-char dimension field plus its padding, so a detail-mode label was cut at ~47 source characters against this row's 60, and the `…` landed mid-field, destroying the column grid. Truncation now happens at both detail call sites (winner row and `also:` competitor row) BEFORE padding, which restores this row's own 'bounds source chars' property in detail mode and keeps padding out of the budget | NOT OBSERVABLE | NOT OBSERVABLE | 19-04  **Run A/B (19-08 Task 3):** Run A NOT OBSERVABLE (no content rendered); Run B NOT OBSERVABLE (the observed labels ('Convective Slight', 'Flash Flood Marginal') were well under the 60-char threshold; truncation was never exercised). |
...
| 39 | 19-REVIEW WR-01 | The empty-render exit is a three-way split: `(unconfirmed)` for a stale or malformed-summary payload, `(filtered by settings)` for a fresh confirmed payload whose content this instance's own display gates hid, and the bare confident string when there was genuinely nothing to show. Case 1 outranks case 2 | (no old site — the legacy renderer had a single unconditional `"No Severe Weather Risk (unconfirmed)"` fallback, `9143705:MMM-SPCOutlook.js:781`) | MMM-SPCOutlook.js — `NO_HAZARD_TEXT_FILTERED` beside its two siblings, `anyUngatedContent()` as the discriminator, and the three-branch `if` at the contentMarker fallback. **This row exists because rows 36-38 created the state it describes:** restoring the per-product gates turned "fresh, confirmed, everything hidden by config" from rare into ordinary, and the old unconditional `(unconfirmed)` spent CR-01's load-bearing word on a clean poll. Structurally-unrenderable content (a wholly-elapsed window entry, a suppressed-only day) is deliberately NOT case 2 — it is unrenderable under any config, so blaming settings would be a lie in the other direction | NOT OBSERVABLE | NOT OBSERVABLE | 19-REVIEW WR-01 fix — added post-sign-off, operator decision on wording (`"No Hazards Forecast (filtered by settings)"`, verbatim). Run A/B NOT OBSERVABLE: neither manual run turned a product toggle off while that product had live content, so the middle case was never on screen. Probe-proven (Probe Coverage row 39), mutation-verified RED four ways (inverted precedence, dropped branch, day-grid-only discriminator, gated discriminator). |
| 40 | 19-REVIEW WR-02 | `summary.enabledSourceCount` is diagnostic-only; the `"No Products Enabled"` empty state that read it is retired, and the field is still emitted | MMM-SPCOutlook.js (RPT-05/18 D-16 empty-state ladder, first branch) | Branch DELETED from `getDom()`; `node_helper.js`'s `_buildGridSummary` keeps emitting the field with an inline note marking it diagnostic-only. The branch was unreachable **in principle**, not merely unreached: `_buildSourceHealth`'s `isAlwaysOn` marks `spc-convective` and `spc-fire` `enabled: true` unconditionally, and neither has a `configFlag` in productRegistry.js or a flag in this file's `defaults`, so the count has a hard floor of 2. The operator declined to make the always-on core configurable purely to make a display branch reachable. D-16 locks the seven flat summary fields, so the FIELD stays — a display fix must not silently break a payload contract | NOT OBSERVABLE | NOT OBSERVABLE | 19-REVIEW WR-02 fix — added post-sign-off, operator decision. Run A/B NOT OBSERVABLE (the branch could never fire on hardware either — that is the finding). The probe that used to "cover" this branch hand-set `enabledSourceCount = 0`, a payload the backend cannot emit, and has been replaced rather than deleted (Probe Coverage row 40). |
...
| 22 | `truncateHazardLabel`/60-char truncation before escaping | `wr02-unified-compact-segment-escapes-hostile-label-and-rejects-hostile-color` (its second assertion: a 200-char label truncates to exactly 55 source chars + `…`, verified truncate-before-escape) , `wr04-detail-label-truncation-counts-source-characters-not-padding` (the detail-mode half: the same 60-source-character bound on both the winner row and the `also:` competitor row, with the dimension field intact ahead of the ellipsis) |
