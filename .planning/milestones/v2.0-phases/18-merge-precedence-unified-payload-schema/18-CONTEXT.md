...
- **D-20: band and advisory counters live in a nested `summary.bandDiagnostics`, not as flat fields.** Decided during planning, after the plan-checker flagged that the planner had widened D-16's locked seven-field shape on its own authority. D-16's seven fields stay flat and exactly as locked; `windowBandCount` and `advisoryCount` sit under `summary.bandDiagnostics` so the extension is obvious to anyone diffing against D-16.
  - Two things this decision separates that were being conflated: `anyHazard` becoming a **union** over days, the window band and advisories is *not* an extension — it is what D-16's stated purpose (make RPT-05 decidable in one read) requires, since a day-scoped-only `anyHazard` would reproduce Phase 15's MPD-invisible gate and the never-exercised Phase 16 band-only case. Only the two **counters** were the addition.
  - Rejected: dropping the counters (Phase 19 could count the legacy arrays that D-01 preserves — but only until Phase 19 deletes them); keeping them flat alongside D-16's seven (silently widens a locked shape).
