...
**Phases:** 14–19.1 (7 phases, one inserted)
**Total Plans:** 67 (175 tasks)
**Timeline:** 2026-08-15 → 2026-09-12 (28 days, 600 commits)
**Requirements:** 37/37 Complete
**Audit:** `v2.0-MILESTONE-AUDIT.md` — status `tech_debt`, 0 blockers

## Overview

Extend the module beyond SPC to WPC and CPC hazard products, and restructure the display from
per-product row sections into a unified per-day report that merges and deduplicates all sources.
...
|-------|----------------|--------|-----------|
| 14. Foundation & WPC Excessive Rainfall Outlook | 7/7 | Complete | 2026-08-23 |
| 15. WPC Winter Storm Severity & Mesoscale Precipitation Discussion | 9/9 | Complete | 2026-08-24 |
| 16. WPC Day 3–7 / CPC Day 8–14 Hazards Outlook | 8/8 | Complete | 2026-08-27 |
| 17. NWS/WPC HeatRisk & Parallelized Fetching | 9/9 | Complete | 2026-09-01 |
| 18. Merge, Precedence & Unified Payload Schema | 16/16 | Complete | 2026-09-06 |
| 19. Unified Day Report — getDom() Rewrite | 9/9 | Complete | 2026-09-07 |
| 19.1. Day Report Render Width, Product-Neutral Copy & Config Key Validation (INSERTED) | 9/9 | Complete | 2026-09-12 |
...
- Phase 19.1: Day report render width, product-neutral copy, and config key validation — inserted
  after Phase 19 to close the three defects Phase 19's UAT found (19-UAT.md tests 5, 11, 13). No
  REQ-IDs; traced to `19.1-CONTEXT.md` D-01..D-06.

**Key Decisions:**

- Product descriptors live in a standalone `productRegistry.js` keyed by product id, never inlined
  into `node_helper.js`'s already-oversized `getSpcOutlook` (D-07).
- `buildArcGisQuery` is a fixed 3-parameter template with `f=geojson` as a hardcoded literal,
  host-allowlisted and integer-guarded on `layerId` (D-09).
- Payload shape is decoupled from `extended` — the backend always emits one shape; `extended` gates
  which fetches run, not what the payload looks like.
- Advisory liveness is decided by each candidate's own `ValidEndTi`, never by its filename number.
  Live-reproduced against `MPD_1281_final.kmz`, an 8-month-stale straggler.
- Precedence and deduplication are computed once in the backend and shipped precomputed; the
  display recomputes nothing.
- The day report is the single render path. No `legacyLayout` toggle — a third render state would
  need permanent maintenance and double UAT.
- Column alignment uses CSS grid tracks, not character-advance widths. The region cap is derived
  available-space-first (`1020 − 540 − 30 = 450px` → `REGION_CAP_REM = 22.5`), not from content
  needs.
- Verification strategy is manual UAT plus static analysis plus a mutation-proven offline probe
  suite; `workflow.nyquist_validation` stays disabled.

**Issues Resolved:**

- SPC MD polls returned zero results in production — an `http://`-vs-`https://` prefix-match defect
  in the advisory URL allowlist, fixed by hostname parsing.
- The stale indicator false-flagged the Hazards Outlook across weekends; it now understands the
  product's weekday-only issuance cadence (84h budget).
- Cold-start fetch time grew linearly with each enabled product; all new fetches were parallelized
  and PERF-01 closed against a measured number.
- Detail-mode day report widened its MagicMirror region and overlapped the centre column
  (19-UAT test 5, MAJOR) — fixed by replacing the alignment mechanism, confirmed on hardware.
- User-facing copy named SPC alone for a six-product aggregator; `getHeader()` gained a three-state
  fallback and the loading string went product-neutral.
- Unrecognised config keys were silently ignored; `start()` now warns with a nearest-key
  suggestion, never logs config values, and never throws.

**Issues Deferred:**

- Legacy payload block emission and probe-suite migration (todo, deferred by operator 2026-09-07).
  Not a pure deletion — the RPT-04 window band is produced inside the legacy `hazardsOutlook`
  block; 3 helpers must be extracted first, and `advisories` is the sole transport for the MPD/SPC
  MD band.
- Pinning `fetchGeoJsonCached`'s network-error hard-failure branch (todo, 2026-09-11).
- 23 live-observation deferrals across Phases 14–19.1 — seasonal (WSSI), event-gated (MPD/ERO), or
  location-gated products whose data condition did not exist during the verification window.
- Phase 16's `16-VERIFICATION.md` stays `human_needed`: 5/5 must-haves verified code-side and
  mutation-proven, 3 live observations outstanding for documented, non-fabricated reasons.
...
- The SPC inline chain is serial and dominates cold-cache latency; PERF-01 scoped only the new
  product fetches.
- Legacy `heatRisk.day1..day7` drops day 7 for ~12h of every 24h, and legacy `hazardsOutlook.dayN`
  uses raw 0-based offset labelling. Both are inert on screen but live in the emitted payload.
- Multi-instance cache/broadcast isolation is unreachable single-instance; it becomes live on a
  second instance or location.
