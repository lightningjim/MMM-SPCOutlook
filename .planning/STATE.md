...
| Operational | Restore the deployed MagicMirror's `lat`/`lon` and config to production values after the Run A/Run B/supplementary substitute-coordinate testing on 2026-09-07 | **NOT CONFIRMED as done** — verify on `ssh mm` before treating the live mirror as production-configured again | Phase 19-08 |
